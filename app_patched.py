...
def run_embedded_app(app_key: str):
    ...
    try:
        ...
        if hasattr(module, "render") and callable(getattr(module, "render")):
            ...
            st.warning("이 앱은 render() 또는 main() 진입점이 없습니다.")
...
with st.sidebar:
    st.sidebar.markdown(
        ...
        .mso-brand-btn button:hover p{text-decoration:none !important;opacity:0.92 !important;}
        ...
        .mso-badge{display:inline-flex;align-items:center;justify-content:center;
            min-width:34px;height:18px;font-size:9px;font-weight:600;color:white;
    ...
    # Active page is shown as a filled white pill for clearer state
    for p in PAGES:
        pid = p['id']
        is_active = (pid == st.session_state['page'])
        c1, c2 = st.sidebar.columns([0.82, 0.18], gap='small')
        if is_active:
            c1.markdown(f"<div class='mso-active'>{p['label']}</div>", unsafe_allow_html=True)
        else:
